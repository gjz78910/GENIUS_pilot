...
def _calculate_job_timings(
    ...
    """Calculate start/end times for jobs in chronological order.

    Returns list of job records with timing information in minutes,
    sorted by job_time with travel calculated between consecutive locations.
    """
    sorted_jobs = sorted(jobs, key=lambda j: j.time)

    current_time_minutes = 0.0
    job_records = []
    prev_loc = engineer.location

    for job in sorted_jobs:
        travel_hours = travel_matrix.get(prev_loc, {}).get(job.location, 0.0)
