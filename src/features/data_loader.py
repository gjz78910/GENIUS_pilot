...
def load_data(file_path: str) -> Tuple[List[Engineer], List[Job], Dict[str, Dict[str, float]]]:
    ...
    # Diagonal must be zero (travel from a location to itself takes no time)
    for loc in travel_matrix:
        if loc in travel_matrix[loc] and travel_matrix[loc][loc] != 0.0:
            raise ValueError(
                f"travel_matrix diagonal must be 0.0; got travel_matrix[{loc}][{loc}]="
                f"{travel_matrix[loc][loc]}"
            )

    # Matrix must be symmetric (d(A,B) == d(B,A))
    for src, dests in travel_matrix.items():
        for dst, t_fwd in dests.items():
            if dst in travel_matrix and src in travel_matrix[dst]:
                t_rev = travel_matrix[dst][src]
                if abs(t_fwd - t_rev) > 1e-9:
                    raise ValueError(
                        f"travel_matrix is not symmetric: "
                        f"[{src}][{dst}]={t_fwd} != [{dst}][{src}]={t_rev}"
                    )
    ...
    for j_data in data["jobs"]:
        ...
        try:
            ...
            raise ValueError(
                f"Job {job_id}: invalid time format '{time_str}'; expected HH:MM (00:00-23:59)"
            )
