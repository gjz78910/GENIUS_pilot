...
for _ in $(seq 1 40); do
  if command -v wmctrl >/dev/null 2>&1; then
    if wmctrl -r "PARTICIPANT_INSTRUCTIONS_AI.html" -b add,maximized_vert,maximized_horz 2>/dev/null; then
      browser_ready=1
    fi
    if wmctrl -r "GENIUS_pilot" -b add,maximized_vert,maximized_horz 2>/dev/null; then
      code_ready=1
    fi
    if [ "$code_ready" -eq 1 ] && [ "$browser_ready" -eq 1 ]; then
      exit 0
    fi
  fi
  sleep 0.5
