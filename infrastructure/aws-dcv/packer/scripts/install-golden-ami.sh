...
fit_window_to_display() {
  local title="$1" screen width height usable_height
  ...
  if [ "$usable_height" -gt 120 ]; then
    usable_height=$((usable_height - 27))
  fi
  wmctrl -r "$title" -e "0,0,27,$width,$usable_height" 2>/dev/null
